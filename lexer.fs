...
module lexer =
  ...
  //
    ...
    let rec private realLiteral nextc input literal = 
        if (List.contains nextc ['0'..'9']) then
            realLiteral (nextChar input) input (literal + (string nextc))
        else
            (literal,nextc,true)
    ...
    let rec private lexer nextc input tokens = 
        ...
        |  _  when List.contains nextc ['0'..'9'] ->
                ...
                let (literal,lookahead,real) = collectIntLiteral (nextChar input) input (string nextc)
                if real then
                    lexer lookahead input (("real_literal:" + literal) :: tokens)
                else
                    lexer lookahead input (("int_literal:" + literal) :: tokens)
                // if(lookahead = '.') then
                //     let (realLit,lookahead1) = collectIntLiteral(nextChar input) input (literal + string lookahead)
                //     lexer lookahead1 input (("real_literal:" + realLit) :: tokens)
                // else
                //     lexer lookahead input (("int_literal:" + literal) :: tokens)
